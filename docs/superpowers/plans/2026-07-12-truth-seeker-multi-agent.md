# truth-seeker 多 Agent 架构实施计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 将 truth-seeker 从单 agent 改为编排者 + 研究员 Agent + 报告生成 Agent 的双 agent 流水线架构，省去终端文本输出，直接生成 HTML 并打开浏览器。

**Architecture:** SKILL.md 充当轻量编排者，依次调用 Agent 1（研究员，负责搜索+审计）和 Agent 2（报告生成，负责推理+HTML），中间通过结构化 JSON 数据契约传递。blocks/ 模板块不变，由 Agent 2 读取组装。

**Tech Stack:** Markdown（SKILL.md）、HTML/CSS（报告模板）、Claude Code Agent 工具

## 文件结构

| 文件 | 职责 |
|------|------|
| `plugins/truth-seeker/skills/truth-seeker/SKILL.md` | 编排者：调度两个 Agent，传递数据，保存并打开 HTML |
| `plugins/truth-seeker/skills/truth-seeker/references/blocks/` | 不变：HTML 模板块，Agent 2 按需读取 |
| `plugins/truth-seeker/skills/truth-seeker/references/blocks/index.md` | 不变：块索引和变量参考 |

Agent 1 和 Agent 2 的指令内联在 SKILL.md 中，不需要单独的 agent 定义文件。

## 全局约束

- 两个 Agent 通过结构化 JSON 传递数据，JSON schema 必须严格遵守
- Agent 1 只搜索和整理，不做推理判断
- Agent 2 只推理和生成 HTML，不做新搜索（除非发现明显证据缺口）
- 最终输出仅为 HTML 文件，不再输出终端纯文本报告
- blocks/ 目录下的模块化模板保持向后兼容，不作修改
- 搜索必须覆盖中英文双语来源

---

### Task 1: 重写 SKILL.md —— 编排者 + Agent 1 研究员 + Agent 2 报告生成

**Files:**
- 重写: `plugins/truth-seeker/skills/truth-seeker/SKILL.md`

**Interfaces:**
- Produces: Agent 1 输出 JSON（数据契约），Agent 2 消费该 JSON 并输出 HTML 文件路径

这是本次变更的核心任务。旧的 SKILL.md（434行）被完全重写为编排者架构。
新文件划分如下：

| 区域 | 行范围 | 内容 |
|------|--------|------|
| 元数据 | 1–6 | frontmatter 和 description |
| §1 核心理念 | ~8–25 | 三条铁律（精简） |
| §2 触发方式 | ~27–40 | 保持不变 |
| §3 编排者工作流 | ~42–80 | **新**：三步编排指令 |
| §4 数据契约 | ~82–140 | **新**：Agent 间 JSON Schema |
| §5 Agent 1 研究员 | ~142–270 | **新**：Agent 1 完整 prompt |
| §6 Agent 2 报告生成 | ~272–450 | **新**：Agent 2 完整 prompt |
| §7 关键边界规则 | ~452–470 | 保留原有规则 |
| §8 信息来源参考 | ~472–490 | 原有的可信度等级表 |
| §9 特殊场景处理 | ~492–520 | 原场景 1–5 精简版 |

- [ ] **Step 1: 备份当前 SKILL.md**

```bash
cp plugins/truth-seeker/skills/truth-seeker/SKILL.md .backup/SKILL-v1.0.1-$(date +%Y%m%d).md
```

- [ ] **Step 2: 写入新 SKILL.md 的元数据 + 核心理念 + 触发方式 + 编排者工作流**

写入文件头部（对应新文件约行 1–80），内容如下：
...
将 Agent 2 返回的 HTML 内容写入文件，然后自动用浏览器打开：

```bash
# 文件名格式：寻真报告_<简短主题>.html，保存到工作目录
start <文件名>.html
```

Agent 2 会在其返回结果的最后一行给出文件名的建议（不含路径），编排者使用该建议。
...
- [ ] **Step 3: 写入 §5 Agent 1 研究员指令**

在上述内容之后追加（对应新文件约行 142–270）：
...
c) 按装配顺序读取所需的 section 块：
   - page-top.html（静态，直接拼接，{{STYLES}} 替换为内联样式）
   ...
   - sidebar-nav.html（静态，直接拼接，只包含实际使用的 section 导航项）
   - about-article.html（推荐：报告元信息 + 引用格式 + 权利声明）
   - page-bottom.html（静态，直接拼接）
...
- 在输出的最后一行给出文件名的建议
- 不要输出 JSON 或任何其他格式——只输出完整的 HTML 内容
...
- [ ] **Step 5: 写入 §7–§9 关键边界规则、信息来源参考、特殊场景处理**
...
读取完整的 SKILL.md 确认结构完整：

```bash
wc -l plugins/truth-seeker/skills/truth-seeker/SKILL.md
```

预期输出：约 380–420 行（比原来的 434 行略短，因为删除了终端输出模板但新增了编排逻辑和 Agent 指令）

- [ ] **Step 7: 提交**

```bash
git add plugins/truth-seeker/skills/truth-seeker/SKILL.md
git commit -m "feat(truth-seeker): 重构为多 Agent 流水线架构——研究员 + 报告生成

- SKILL.md 改为编排者角色，调度两个专职 Agent
- Agent 1（研究员）：拆解陈述 + 多角度双语搜索 + 五维来源审计
- Agent 2（报告生成）：贝叶斯推理 + 竞争假设 + 原理分析 + HTML 生成
- 删除终端文本输出，直接生成 HTML 并打开浏览器
- blocks/ 模板块保持不变，由 Agent 2 读取组装"
```
...
- [ ] **Step 1: 用简单陈述触发 truth-seeker**

在 Claude Code 中执行：
```
/truth-seeker 每天喝八杯水对健康是必要的
```

预期行为：
- 编排者首先展示拆解结果（Agent 1 的拆解部分）
- 编排者调用 Agent 1 研究员
- Agent 1 返回结构化 JSON
- 编排者将 JSON 传给 Agent 2
- Agent 2 生成 HTML 报告
- 编排者保存文件并打开浏览器

- [ ] **Step 2: 检查 Agent 1 产出的 JSON 质量**

验证点：
- `decomposition.claims` 至少有一个主张
- 每个主张有中英文关键词
- `evidence[].supporting` 至少有 2 条证据
- 每条证据的 `audit` 四个维度全部填写
- `search_quality` 四个字段全部填写
- `researcher_note` 非空且用中文

- [ ] **Step 3: 检查 Agent 2 产出的 HTML 质量**

验证点：
- HTML 文件在工作目录下生成
- 浏览器自动打开
- HTML 包含完整结构：标题、可信度评级、逐条验证、原理分析、结论
- 样式正常渲染（内联 CSS 生效）
- 无 TBD 或占位符
- DOI 编号格式正确

- [ ] **Step 4: 如有问题，迭代修复 SKILL.md 中的 Agent 指令**

根据验证结果调整 Agent 1 或 Agent 2 的 prompt 中不清晰或遗漏的部分。
修复后重新提交：

```bash
git add plugins/truth-seeker/skills/truth-seeker/SKILL.md
git commit -m "fix(truth-seeker): 修复 Agent 指令中的 [具体问题]"
```
